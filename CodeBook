...
The dataset data_tidy.txt is derived from the "Human Activity Recognition Using Smartphones Data Set" assigned in the
Getting and Cleaning Data Course Project.


Each file was read individually as text.
The data were combined by subject, activity and x into one data frame that holds the test data and 
another that holds the training data.  

The program subsequently merges the Training and Testing Sets by rows into 1 data set called data.all.
Approximately 10,000 observations in data.all

The code uses descriptive activity names to name the activities in the data set. This is
done by reading the labels from the activity_labels.txt file

The set of summary statistics that were estimated from these signals are:

    mean(): Mean value
    std(): Standard deviation

The tidy variable names are listed below.
The variable names for the signals start at item [3] and the suffix indicates the type of summary statistic. 
